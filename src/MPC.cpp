...
double actuatorFactor = 1;

// Some helper variables to keep track of the indices in our 1-D data vectors.
size_t x_start = 0;
...
public:
	...
	int latencyFrames;
	FG_eval(Eigen::VectorXd coeffs, int latencyFrames) 
	{ 
		this->coeffs = coeffs; 
	...
	void operator()(ADvector& fg, const ADvector& vars) {
		...
		for (size_t t = 1; t < MPC::N; t++)
		{
			...
			AD<double> psides0 = CppAD::atan(coeffs[1] + 2.0 * coeffs_[2] * x0 + 3.0 * coeffs[3] * CppAD::pow(x0, 2));
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, double latencySecs)
{
	bool ok = true;
	...
	FG_eval fg_eval(coeffs, latencyFrames);
